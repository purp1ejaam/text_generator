...
#define END "]"
struct Pairs {
	double probability = 0, count = 0; //частота встречи наиболее вероятных слов 
...
public:
	...
	void compliationOfNewText(std::string word) {
		...
		std::cout << newWord << " ";
		if (newWord != END && marcov.Chain.find(newWord)->second.count != 0) {
			newText.push_back(' ');
			newText = newText + newWord;
			--marcov.Chain.find(newWord)->second.count;
...
/*	for (auto it : k.marcov.Chain.find("и")->second.words)
		std::cout << it.first << " " << it.second << std::endl;
	std::cout << std::endl << "that's all " << std::endl;
	for (auto it : k.marcov.Chain.find("и")->second.actualWords)
